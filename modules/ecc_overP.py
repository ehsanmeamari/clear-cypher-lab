    ...
    with col1:
        ...
        if points_list:
            ...
            with st.expander(f"Points on curve ({len(points_list)+1} points):", expanded=False):
                st.markdown(f"<div style='font-family: monospace; white-space: normal; word-wrap: break-word; overflow-wrap: break-word;'>{{ 𝒪, {', '.join(str_points)} }}</div>", unsafe_allow_html=True)
