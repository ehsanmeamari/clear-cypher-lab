...
def ecc_fp():
    ...
    Includes curve definition, point listing, and arithmetic operations.
    """
    ...
    with col1:
        ...
        # --- Section: Points List (The part you requested) ---
        if points_list:
            str_points = [f"({pt[0]},{pt[1]})" for pt in points_list]
            all_points_str = ", ".join(str_points)
            
            # Note: We add +1 for the Point at Infinity (O)
            with st.expander(f"Points on curve ({len(points_list)+1} points including 𝒪):", expanded=False):
                points_html = f"""
                    <div style='font-family: monospace; font-size: 15px; line-height: 1.6; color: #2c3e50; padding: 5px; word-break: break-all;'>
                        {{ 𝒪, {all_points_str} }}
                    </div>
                """
        ...
        with st.expander("Point Addition", expanded=True):
            ...
            if is_on_curve(P, a, b, p) and is_on_curve(Q, a, b, p):
                ...
                if R is None: st.caption("Result: Point at infinity 𝒪")
            else:
                st.warning("Points must be on the curve.")
